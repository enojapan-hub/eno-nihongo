-- Post-deploy switch for N3/N5 source-backed listening banks and the staged N5 reading map.
-- Run ONLY after the production /api/jlpt-image endpoint has been verified
-- for the N3/N5/N5-reading Drive image IDs added by PR #7.
...
begin
  ...
  select count(*) into n5_reading_map_count
  from public.jlpt_simulation_questions
  where level='N5'
    and section='reading'
    and mondai_no=4
    and display_question_no=3
    and source_kind='drive_reference'
    and image_url='/api/jlpt-image?id=1n7uuOeCPCsvQYrNu9fYKeA2mQzVEsu75';
  ...
  if n5_reading_map_count <> 1 then
    raise exception 'Refusing switch: expected one staged N5 reading map row at 問題4 問3, found %', n5_reading_map_count;
  end if;
  if bad_rows <> 0 then
    raise exception 'Refusing switch: found % malformed N3/N5 source rows', bad_rows;
  end if;
end $$;
...
-- Publish the verified source-backed listening banks.
update public.jlpt_simulation_questions
...
-- Publish the staged N5 reading map after its image endpoint is verified.
update public.jlpt_simulation_questions
set is_published = true
where level='N5'
  and section='reading'
...
-- N5 reading 問題4 = 3 drive_reference rows, including the map question

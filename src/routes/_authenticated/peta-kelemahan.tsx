...
function WeaknessMapPage(){
  ...
  return <AppShell compact title="Peta Kelemahan"><div className="mx-auto w-full max-w-md space-y-3 pb-4">
    ...
    {rows.length>0&&<section className="rounded-3xl border bg-card p-4"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-primary/10"><Gauge className="size-5 text-primary"/></span><div className="flex-1"><p className="text-[9px] font-bold uppercase tracking-wider text-primary">ENO Recall Score</p><p className="text-[11px] font-bold">Kualitas ingatan · {recallLabel}</p><p className="mt-0.5 text-[8px] leading-4 text-muted-foreground">Menggabungkan recall benar, tanpa hint, kecepatan, recall balik, dan konsistensi lintas hari.</p></div><span className="text-[24px] font-black">{recall}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{width:`${recall}%`}}/></div></section>}

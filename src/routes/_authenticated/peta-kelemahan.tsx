...
function WeaknessMapPage(){
  ...
  return <AppShell compact title="Peta Kelemahan"><div className="mx-auto w-full max-w-md space-y-3 pb-4">
    ...
    {membership.data?.plan==='free'?<UpgradeCard title="Peta Kelemahan lengkap untuk Premium" description="Free tetap bisa memakai Hafalan. Premium menampilkan prioritas materi, Recall Score, dan latihan yang disusun dari jawabanmu."/>:rows.length>0&&<section className="rounded-3xl border bg-card p-4"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-primary/10"><Gauge className="size-5 text-primary"/></span><div className="flex-1"><p className="text-[9px] font-bold uppercase tracking-wider text-primary">ENO Recall Score</p><p className="text-[11px] font-bold">Kualitas ingatan · {recallLabel}</p><p className="mt-0.5 text-[8px] leading-4 text-muted-foreground">Menggabungkan recall benar, tanpa hint, kecepatan, recall balik, dan konsistensi lintas hari.</p></div><span className="text-[24px] font-black">{recall}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{width:`${recall}%`}}/></div></section>}
    {membership.data?.plan!=='free'&&(reviews.isLoading?<p className="py-8 text-center text-[10px] text-muted-foreground">Menganalisis riwayat hafalan…</p>:!rows.length?<section className="rounded-2xl border bg-card p-5 text-center"><BrainCircuit className="mx-auto size-7 text-muted-foreground"/><p className="mt-2 text-[11px] font-bold">Belum cukup data</p><p className="mt-1 text-[9px] text-muted-foreground">Selesaikan beberapa kartu di Mode Hafalan agar ENO dapat memetakan kelemahanmu.</p></section>:<div className="space-y-2">{stats.map((x,i)=><section key={x.key} className="rounded-2xl border bg-card p-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-[11px] font-bold">{x.label}</p><p className="text-[8px] text-muted-foreground">{x.total} review · {i===0?'Prioritas utama':'Mastery per aspek'}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-black ${x.mastery<50?'bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300':x.mastery<75?'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300':'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300'}`}>{x.mastery}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{width:`${x.mastery}%`}}/></div>{i===0&&<a href={masteryTrainingHref(x)} className="mt-3 block rounded-xl bg-primary py-2.5 text-center text-[9px] font-bold text-primary-foreground">Latih {x.label}</a>}</section>)}</div>)}

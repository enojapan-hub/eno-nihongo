...
function Certificate() {
  ...
  useEffect(() => {
    ...
    } catch { setProgress(null); } finally { setLoaded(true); }
  }, [level]);
  ...
  return <AppShell title="Sertifikat Simulasi" compact><div className="mx-auto max-w-2xl space-y-3">
    ...
    <div className="rounded-[28px] border-4 border-double border-primary/40 bg-card p-3 shadow-sm"><div className="rounded-[22px] border border-primary/20 p-7 text-center sm:p-10">
      <Award className="mx-auto size-12 text-primary" /><p className="mt-4 text-xs font-bold tracking-[.25em] text-primary">ENO NIHONGO</p><h1 className="mt-3 text-2xl font-black tracking-wide">CERTIFICATE OF ACHIEVEMENT</h1><p className="mt-2 font-jp text-sm font-semibold">日本語能力 模擬試験 合格証明書</p><div className="mx-auto my-6 h-px max-w-sm bg-border" />
      <p className="text-xs text-muted-foreground">Diberikan atas keberhasilan menyelesaikan</p><p className="mt-2 text-lg font-bold">Simulasi Penuh JLPT {level}</p>
      <div className="mx-auto mt-6 grid max-w-md grid-cols-2 gap-3 text-left">
        ...
        <div className="rounded-xl bg-muted/50 p-3"><p className="text-[10px] text-muted-foreground">CEFR REFERENCE</p><p className="mt-1 text-lg font-black text-primary">{result.cefr ?? "—"}</p></div>
        <div className="col-span-2 rounded-xl bg-muted/50 p-3"><p className="text-[10px] text-muted-foreground">TANGGAL SIMULASI</p><p className="mt-1 text-xs font-bold">{date}</p></div>
      </div>
      ...
      <p className="mt-5 text-[10px] leading-5 text-muted-foreground">CEFR adalah reference indication berdasarkan level dan skor total simulasi. Sertifikat digital ini diterbitkan oleh ENO NIHONGO untuk pencapaian dalam simulasi belajar. Bukan sertifikat resmi JLPT, bukan sertifikat CEFR resmi, dan tidak diterbitkan atau disahkan oleh penyelenggara JLPT.</p>
    </div></div>
  </div></AppShell>;
}

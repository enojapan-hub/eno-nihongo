...
async function itemSuggestion(client:any,r:{item_type:string;item_id:string},weak=false,weakLabel?:string):Promise<AdaptiveSuggestion|null>{
  ...
  if(r.item_type==="kanji"){const{data}=await client.from("kanji").select("character,meaning_id").eq("id",r.item_id).maybeSingle();return data?{id:r.item_id,label:String(data.character??"Kanji"),subtitle:`${prefix} · ${data.meaning_id??"Kanji"}`}:null}
  if(r.item_type==="vocabulary"){const{data}=await client.from("vocabulary").select("term,meaning_id").eq("id",r.item_id).maybeSingle();return data?{id:r.item_id,label:String(data.term??"Kosakata"),subtitle:`${prefix} · ${data.meaning_id??"Kosakata"}`}:null}
  if(r.item_type==="grammar"){const{data}=await client.from("grammar_points").select("pattern,meaning_id").eq("id",r.item_id).maybeSingle();return data?{id:r.item_id,label:String(data.pattern??"Bunpou"),subtitle:`${prefix} · ${data.meaning_id??"Bunpou"}`}:null}
  return null;
}
...
async function enrichTasksWithSuggestions(userId:string,level:string,tasks:AdaptiveTask[]):Promise<AdaptiveTask[]>{
  ...
  for(const r of reviews){if(!reviewMatchesWeakness(r,weakest))continue;const k=`${r.item_type}:${r.item_id}`,penalty=(r.rating<2?.65:0)+(r.used_hint?.2:0)+(Number(r.response_ms??0)>8000?.15:0);aspectItems.set(k,(aspectItems.get(k)??0)+penalty)}
  return Promise.all(tasks.map(async task=>{
    ...
    try{
      ...
      if(task.task_type==="new_kanji"){const skip=known("kanji");const{data}=await client.from("kanji").select("id,character,meaning_id").eq("level",level).eq("is_published",true).order("sort_order",{ascending:true}).limit(wanted*4);return{...adjusted,suggestions:(data??[]).filter((x:any)=>!skip.has(String(x.id))).slice(0,wanted).map((x:any)=>({id:String(x.id),label:String(x.character??"Kanji"),subtitle:x.meaning_id?String(x.meaning_id):null}))}}
      if(task.task_type==="new_vocabulary"){const skip=known("vocabulary");const{data}=await client.from("vocabulary").select("id,term,reading,meaning_id").eq("level",level).eq("is_published",true).order("sort_order",{ascending:true}).limit(wanted*4);return{...adjusted,suggestions:(data??[]).filter((x:any)=>!skip.has(String(x.id))).slice(0,wanted).map((x:any)=>({id:String(x.id),label:String(x.term??"Kosakata"),subtitle:[x.reading,x.meaning_id].filter(Boolean).map(String).join(" · ")}))}}
      if(task.task_type==="new_grammar"){const skip=known("grammar");const{data}=await client.from("grammar_points").select("id,pattern,meaning_id").eq("level",level).eq("is_published",true).order("sort_order",{ascending:true}).limit(wanted*4);return{...adjusted,suggestions:(data??[]).filter((x:any)=>!skip.has(String(x.id))).slice(0,wanted).map((x:any)=>({id:String(x.id),label:String(x.pattern??"Bunpou"),subtitle:x.meaning_id?String(x.meaning_id):null}))}}
      if(task.task_type==="reading"){const skip=mastered("reading");const{data}=await client.from("reading_passages").select("id,title").eq("level",level).eq("is_published",true).order("sort_order",{ascending:true}).limit(wanted*3);return{...adjusted,suggestions:(data??[]).filter((x:any)=>!skip.has(String(x.id))).slice(0,wanted).map((x:any)=>({id:String(x.id),label:String(x.title??"Dokkai")}))}}
      if(task.task_type==="listening"){const skip=mastered("listening");const{data}=await client.from("listening_items").select("id,title,duration_seconds").eq("level",level).eq("is_published",true).order("sort_order",{ascending:true}).limit(wanted*3);return{...adjusted,suggestions:(data??[]).filter((x:any)=>!skip.has(String(x.id))).slice(0,wanted).map((x:any)=>({id:String(x.id),label:String(x.title??"Choukai"),subtitle:x.duration_seconds?`${Math.ceil(Number(x.duration_seconds)/60)} menit`:null}))}}
      return adjusted;
    }catch{return task}
  }));
}
